  ...
  function toggleItemOpenEq(items) {
    return items.map((item) => {
      ...
      return item === target ? {
        ...
        isOpen,
        subItems: toggleItemOpenEq(subItems),
        ...others,
      }
    })
  }
...
function filterBookCovers(covers, {
  filter,
  sorting,
}) {
  let sorter
...
let newBookCovers = null
    switch (action.type) {
      ...
      case UPDATE_FILTER:
        ...
        newBookCovers = filterBookCovers(_.values(state.books), {
          filter: action.filter,
      ...
      case UPDATE_SORTING:
        ...
        newBookCovers = filterBookCovers(_.values(state.books), {
          filter: state.filter,
          sorting: _.merge({}, state.sorting, action.order),
        })
        break
      default:
    }
  ...
  reader(state = {}, action) {
    switch (action.type) {
      ...
      case TOGGLE_TOC_PIN:
        state = { ...state,
          isTocPinned: !state.isTocPinned,
        }
        break
      case TOGGLE_TOC_OPEN:
        state = { ...state,
          isTocOpen: action.open == null ? !state.isTocOpen : action.open,
        }
        break
      case TOGGLE_TOC_ITEM_OPEN:
        state = { ...state,
          toc: toggleTocItemOpen(state.toc, action.itemOrAllOpen),
        }
        break
      case UPDATE_READER_PROGRESS:
        const {
          ...
          chapterTitle,
          items,
        } = updateSelectedTocItem(toc, action.progress.chapterPath)
        if (toc === items) {
          state = { ...others,
            toc,
            progress: { ...progress,
...
export function prepareSavedState(state) {
  state.shelf.bookCovers = filterBookCovers(_.values(state.shelf.books), {
    sorting: state.shelf.sorting,
  })
  return state
}
