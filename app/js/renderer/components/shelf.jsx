            ...
            <Menu.Item>
              ...
              <Menu.Menu>
                {['Title', 'Last Read'].map((title, index) => (
                  <Menu.Item
                    active={title === sorting.method}
                    key={index}
                    onClick={() => changeOrder({ method: title })}
                  >
                    {title}
              ...
              <Menu.Menu>
                {['ascending', 'descending'].map((title, index) => (
                  <Menu.Item
                    ...
                    active={title === sorting.order}
                    key={index}
                    onClick={() => changeOrder({ order: title })}
                  >
                    {title}
...
export const ShelfBody = ({
  ...
  <div
    id="books-shelf"
    style={
      customMargin ? { paddingLeft: viewMargin, paddingRight: viewMargin } : {}
    }
  >
    {bookCovers.map((book, index) => (
      <div
        ...
        style={
          customMargin
            ? { marginLeft: bookMargin / 2, marginRight: bookMargin / 2 }
            : {}
        }
      >
          ...
          <div className="cover-border">
            {book.coverImage ? (
              <img
                ...
                src={`ebook://doc.${book.id || ''}/?${querystring.stringify({
                  s: 'cover',
                  p: book.coverImage.href,
                  mt: book.coverImage.mediaType
                })}`}
              />
            ) : null}
          </div>
        </div>
        <label className="book-title" title={book.fileName}>
          {book.title}
        </label>
      </div>
    ))}
