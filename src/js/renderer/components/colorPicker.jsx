		...
		<div className='color-picker-popup'>
			<div className={activePanel === 'material' ? 'material panel' : 'hide'}>
			{
				...
				<div key={index} className='column'>
				{
					...
					<div key={index}
						className={colorStyles.backgroundColor === upHex ? 'selected item' : 'item'}
						style={{ ...colorStyles, borderColor: colorStyles.color }}
						onClick={() => this.onChanged(colorStyles.backgroundColor)}>
						<Icon name='check' />
					</div>
					))
				}
				...
				<div className={'#000000' === upHex ? 'selected special item black' : 'special item black'}
					onClick={() => this.onChanged('#000000')}>
					<Icon name='check' />
				</div>
				<Button className='switch-button' basic icon onClick={() => this.setState({activePanel: 'free'})}>
					<Icon name='exchange' />
				</Button>
			</div>
...
}, (v) => _.map(v, ({color, text}) => ({ backgroundColor: color.toUpperCase(), color: text })) )
